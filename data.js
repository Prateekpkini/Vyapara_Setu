// VyaparSetu — Seed Data & Shared Utilities
// Inject this via <script src="data.js"> on all pages
...
// Seed trial data into localStorage if not present
function seedData() {
  let users = JSON.parse(localStorage.getItem('vs_users') || '[]');
  const existingIds = users.map(u => u.id);
  let added = 0;
  TRIAL_DATA.forEach(u => {
    if (!existingIds.includes(u.id)) {
      users.push(u);
      added++;
    }
  });
  if (added > 0) localStorage.setItem('vs_users', JSON.stringify(users));
}

// Get current session
function getSession() {
  return JSON.parse(localStorage.getItem('vs_session') || 'null');
}
...
function scoreColor(score) {
  if (score >= 700) return '#00c896';
  if (score >= 550) return '#ff8c00';
  return '#e63946';
}

// Grade badge
function gradeBadge(grade, risk) {
  const colors = { A: '#00c896', B: '#4CAF50', C: '#ff8c00', D: '#ff6b35', F: '#e63946' };
  return `<span style="background:${colors[grade]}22;color:${colors[grade]};border:1px solid ${colors[grade]}44;padding:3px 10px;border-radius:100px;font-size:11px;font-weight:700;">Grade ${grade} · ${risk}</span>`;
}

// Expose globally
window.VS = { seedData, getSession, getAllUsers, getUserById, getCurrentUser, logout, formatINR, scoreColor, gradeBadge, TRIAL_DATA };
